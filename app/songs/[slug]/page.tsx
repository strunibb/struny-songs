...
import { ELECTRIC_GUITAR_SECTION, levelMeta, UNASSIGNED_LEVEL, type SongLevel } from "@/lib/song-types";
...
export default async function SongPage({ params }: Props) {
  ...
  const meta = thematicOnly ? { color: electricOnly ? "blue" : "purple" } : levelMeta[song.level as SongLevel];
  ...
  const featureIcons: Record<string, string> = { Аккорды: "♬", Бой: "↯", Перебор: "≋", Рифф: "ϟ", Соло: "◇", Фингерстайл: "✦", Табулатура: "▦", Электрогитара: "⚡" };
  return (
    <main className="song-page">
      <SiteHeader />
      <section className="song-detail"><div className="shell"><div className="breadcrumbs"><Link href="/">Главная</Link><span>/</span><Link href="/#catalog">Каталог</Link><span>/</span><b>{song.artist} — {song.title}</b></div><div className="detail-grid"><SongCover song={song} large /><div className="detail-copy"><span className={`level-badge level-${meta.color}`}><i>●</i>{thematicOnly ? (electricOnly ? ELECTRIC_GUITAR_SECTION : "Интересный бой") : song.level}</span><p className="artist-name">{song.artist}</p><h1>{song.title}</h1><p className="detail-description">{song.description}</p><div className="detail-facts"><span><small>Сложность</small><strong>{"●".repeat(song.difficulty)}<i>{"●".repeat(4 - song.difficulty)}</i></strong></span><span><small>Тональность</small><strong>{song.keyName || "Оригинал"}</strong></span><span><small>Баррэ</small><strong>{song.barre ? "Есть" : "Нет"}</strong></span></div><div className="purchase-panel"><div><small>Выберите формат</small><strong>Добавить в корзину</strong></div><PurchaseOptions song={song} /></div><p className="safe-note">✓ Заявка со списком песен и выбранными форматами откроется в Telegram</p></div></div></div></section>
