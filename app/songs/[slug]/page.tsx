...
import { levelMeta } from "@/lib/song-types";
...
export default async function SongPage({ params }: Props) {
  const song = await findPublicSong((await params).slug);
  if (!song) notFound();
  const meta = levelMeta[song.level];
  ...
  return (
    <main className="song-page">
      <SiteHeader />
      <section className="song-detail"><div className="shell"><div className="breadcrumbs"><Link href="/">Главная</Link><span>/</span><Link href="/songs">Разборы</Link><span>/</span><b>{song.artist} — {song.title}</b></div><div className="detail-grid"><SongCover song={song} large /><div className="detail-copy"><span className={`level-badge level-${meta.color}`}><i>●</i>{song.level}</span><p className="artist-name">{song.artist}</p><h1>{song.title}</h1><p className="detail-description">{song.description}</p><div className="detail-facts"><span><small>Сложность</small><strong>{"●".repeat(song.difficulty)}<i>{"●".repeat(5 - song.difficulty)}</i></strong></span><span><small>Тональность</small><strong>{song.keyName || "Не указана"}</strong></span><span><small>Каподастр</small><strong>{song.capo || "Не указан"}</strong></span><span><small>Баррэ</small><strong>{song.barre ? "Есть" : "Нет"}</strong></span></div><div className="purchase-panel"><div><small>Полный разбор</small><strong>{song.price} ₽</strong></div><TelegramCta artist={song.artist} title={song.title} price={song.price} /></div><p className="safe-note">✓ После оплаты Никита лично отправит все материалы в Telegram</p></div></div></div></section>
      ...
      <SiteFooter />
      <div className="mobile-buy"><div><small>{song.artist}</small><strong>{song.price} ₽</strong></div><TelegramCta artist={song.artist} title={song.title} price={song.price} compact /></div>
    </main>
  );
}
