        ...
        {showForm ? (
          <section className="admin-editor">
            ...
            <form key={current?.id ?? "new"} onSubmit={submit}>
              <div className="form-section"><h3>Основное</h3><div className="form-grid two"><label>Исполнитель *<input name="artist" defaultValue={current?.artist} required placeholder="Кино" /></label><label>Название песни *<input name="title" defaultValue={current?.title} required placeholder="Пачка сигарет" /></label><label>Раздел<select name="level" defaultValue={current?.level ?? UNASSIGNED_LEVEL}>{ADMIN_LEVELS.map((level) => <option key={level}>{level}</option>)}</select></label><label>Цена, ₽<input name="price" type="number" min="0" defaultValue={current?.price ?? 199} /></label></div><label>Описание<textarea name="description" rows={4} defaultValue={current?.description} placeholder="Что ученик освоит в этом разборе" /></label></div>
