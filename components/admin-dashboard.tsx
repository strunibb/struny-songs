...
import { FEATURE_OPTIONS, LEVELS, type AdminSong } from "@/lib/song-types";
  ...
  return (
    <main className="admin-page">
      ...
      <div className="admin-shell">
        ...
        {showForm ? (
          <section className="admin-editor">
            ...
            <form key={current?.id ?? "new"} onSubmit={submit}>
              <div className="form-section"><h3>Основное</h3><div className="form-grid two"><label>Исполнитель *<input name="artist" defaultValue={current?.artist} required placeholder="Кино" /></label><label>Название песни *<input name="title" defaultValue={current?.title} required placeholder="Пачка сигарет" /></label><label>Уровень<select name="level" defaultValue={current?.level ?? "Начинающий"}>{LEVELS.map((level) => <option key={level}>{level}</option>)}</select></label><label>Цена, ₽<input name="price" type="number" min="0" defaultValue={current?.price ?? 199} /></label></div><label>Описание<textarea name="description" rows={4} defaultValue={current?.description} placeholder="Что ученик освоит в этом разборе" /></label></div>
        ...
        <section className="admin-list"><div className="admin-list-tools"><div className="admin-search">⌕<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Поиск по песням" /></div><span>Показано: {visibleSongs.length}</span></div><div className="admin-table"><div className="admin-row admin-table-head"><span>Название</span><span>Категория</span><span>Цена</span><span>Статус</span><span>Действия</span></div>{visibleSongs.map((song) => <div className="admin-row" key={song.id}><div><div className={`admin-mini-cover cover-${song.coverStyle}`}>{song.coverUrl ? <img src={song.coverUrl} alt="" /> : song.artist.slice(0, 1)}</div><span><strong>{song.title}</strong><small>{song.artist}</small></span></div><span>{song.level}</span><b>{song.price} ₽</b><span className={song.status === "published" ? "status-live" : "status-draft"}><i />{song.status === "published" ? "Опубликован" : "Черновик"}</span><div className="row-actions"><button onClick={() => edit(song)}>Редактировать</button><button onClick={() => toggleVisibility(song)}>{song.status === "published" ? "Скрыть" : "Опубликовать"}</button><button className="danger" onClick={() => remove(song)}>Удалить</button></div></div>)}</div></section>
      </div>
    </main>
  );
}
